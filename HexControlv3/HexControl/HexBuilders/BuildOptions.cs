...
namespace HexBuilders
{
    public class StandardFunctions
    {
        ...
        public byte[] MoveStringBuilder(int numFrame, double x, double y, double z, double u, double v, double w)
        {
        ...
        public static string MoveInterpret(byte[] recBuffer)
        {
        ...
        public byte[] RegistryReadStringBuilder(string RegWriter, int StartAddress, int NumberParameters)
        {
            StringBuilder str = new StringBuilder();
        ...
        /// </summary>
        /// <returns>A StringBuilder</returns>
        public byte[] RegistryWriteStringBuilder(string RegType, int StartAddress,
                                                 int NumberParameters, int ParamCounter, int[] ParamValues)
        {
            StringBuilder str = new StringBuilder();
        ...
        public static string ReadRegistryInterpret(byte[] recBuffer)
        {
            var temp_string = BitConverter.ToString(recBuffer);
        ...
        public static string WriteRegistryInterpret(byte[] recBuffer)
        {
            var temp_string = BitConverter.ToString(recBuffer);
    ...
    class CommandClass : StandardFunctions
    {
        ...
        public string Interpret(byte[] recBuffer)
        {
            var temp_string = BitConverter.ToString(recBuffer);
            var x = Convert.ToInt32(temp_string.Substring(59, 12).Replace("-", ""), 16) * 5f / 10000 - 5;
            var y = Convert.ToInt32(temp_string.Substring(71, 12).Replace("-", ""), 16) * 5f / 10000 - 5;
        ...
        private StringBuilder strBldr(int numFrame, double _xPos, double _yPos, double _zPos, double _uPos, double _vPos, double _wPos)
        {
            StringBuilder str = new StringBuilder();
        ...
        /// <summary>
        /// Input individual axis positions (mm), Output Byte Array
        /// </summary>
        /// <param name="x">Motor 1</param>
        /// <param name="y">Motor 2</param>
        /// <param name="z">Motor 3</param>
        /// <param name="u">Motor 4</param>
        /// <param name="v">Motor 5</param>
        /// <param name="w">Motor 6</param>
        /// <returns>A Byte Array Formatted for Hexapod Communication</returns>
        public byte[] MoveCommand(double x, double y, double z, double u, double v, double w)
        {
            var errTracker = (x >= 450) || // Verifies that max arm length is not exceeded
                             (y >= 450) || // Verifies that max arm length is not exceeded
                             (z >= 450) || // Verifies that max arm length is not exceeded
    ...
    class RegistryClass : StandardFunctions
    {
        ...
        /// </summary>
        /// <returns>A StringBuilder</returns>
        private StringBuilder strBldr(string ReadWrite, string RegWriter, int StartAddress,
                                      int NumberParameters, int ParamCounter, int[] ParamValues)
        {
            StringBuilder str = new StringBuilder();
        ...
        /// <summary>
        /// Input registry information, Output a byte array to send
        /// </summary>
        /// <param name="ReadWrite">Read or Write a Register</param>
        /// <param name="RegisterNumber"> How to write to a Register</param>
        /// <param name="StartAddress"> What register to R/W (int)</param>
        /// <param name="NumberParameters">How many Registers to read (int)</param>
        /// <param name="ParamCounter">When writing, how many parameters are you changing (int)</param>
        /// <param name="ParamValues">An int[] of all the new parameter values</param>
        /// <returns>A Byte Array Formatted for Hexapod Communication</returns>
        public byte[] RegistryCommand(string ReadWrite, string RegisterNumber, int StartAddress,
                                      int NumberParameters, int ParamCounter, int[] ParamValues)
        {
            StringBuilder str;
            // Builds the HEX String
            str = strBldr(ReadWrite, RegisterNumber, StartAddress,
                          NumberParameters, ParamCounter, ParamValues);
        ...
        public string Interpret(byte[] recBuffer)
        {
            ...
            List<Registers> RegList = AllRegisters();
            sAddress = "0x" + sAddress;
            var value = RegList.First(item => item.RegisterNumber == Convert.ToUInt16(sAddress, 16));
        ...
        private List<Registers> AllRegisters()
        {
            List<Registers> RegList = new List<Registers>();
            RegList.Add(new Registers("FN", 0x000A, "0, 1", "1", 0, "How MBOX will STOP on FAULT: \r\n 0: Servo enabled, TURN OFF, STOP \r\n 1: Servo KEEP LOCATION, S/D"));
            RegList.Add(new Registers("FN", 0x000B, "10 - 10,000", "500 pulses", 0, "Error band for INITIAL position"));
    ...
    //public class MatlabClass
    //{
    //    UDPHex UDPConnection;
    //    CommandClass commandClass = new CommandClass();

    //    public void Connect()
    //    {
    //        UDPConnection = UDPHex.Connect();
    //    }

    //    public async Task<byte[]> SendCommand(double x, double y, double z, double u, double v, double w)
    //    {
    //        var response = await UDPConnection.UdpSendReceive(commandClass.MoveCommand(x, y, z, u, v, w));// builder.MoveCommand(x, x, x, x, x, x));

    //        return response;
    //    }

    //    public string Interpret(byte[] response)
    //    {
    //        return commandClass.Interpret(response);
    //    }
