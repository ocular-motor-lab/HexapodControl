function [allMovement] = create_move_array(Duration, RotDeg, Freq, psi_flag, theta_flag, phi_flag)
...
% true limit is in the mm/ms velocity of 1.3mm/10ms
...
for i = RotMovement
    [ArmExtensionMMs,validL,~,~,~,~,~] = CalculateArmLength(X, Y, Ending,...
                                         i * phi_flag, i * theta_flag, i * psi_flag,... % phi, theta-yaw, psi-roll (i)
                                         RefX, RefY, RefZ);
    if validL
%         cmdInvalid = hexControl.SendCommand(ArmExtensionMMs(1),...
%                                       ArmExtensionMMs(2),...
